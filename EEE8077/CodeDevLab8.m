function plot_16QAM_BER()
    ...
    k       = 128;   % Cyclic Prefix Length (could be 0, 16, 32, or 64)
    ...
    %parfor m = 0:1:max_dB/step_dB % parallel loop: swappable with regular
        ...
        for n = numsims:-1:1
            ...
            z      = Channel(x, N+k, sv(snr+1), h); % Send thru noisy channel
            b_est  = Receiver(z, N, k, C, H); % Retrieve info at other end
    ...
    BERPlot(BER_list, snr_dB, time, N, H);
end
...
function BERPlot(BER, dBRange, time, N, H)
    ...
    semilogy(dBRange, T_BER_16QAM_in_MP,'r-');  % plotted for comparison
    hold on;
